  ...
  const fetchEvents = async (pageNumber = 1, searchValue = "") => {
    try {
      ...
      const res = await axios.get(
        `https://bk-production-d11b.up.railway.app:5000/events?page=${pageNumber}&limit=6&search=${searchValue}`,
      );
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {events.map((event) => (
          <div
            ...
            key={event.id}
            className="group rounded-2xl p-6 shadow-lg border border-white/10 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl bg-gray-800"
          >
            <div className="flex items-center gap-3 mb-4">
              <Avatar size="md">
                <AvatarFallbackText>{event.title}</AvatarFallbackText>

                <AvatarImage source={event.photos?.[0]} alt={event.title} />
              </Avatar>
