  ...
  const fetchParticipants = async (pageNumber = 1, searchValue = "") => {
    try {
      ...
      const res = await axios.get(
        `https://bk-production-0584.up.railway.app/participants/participants?page=${pageNumber}&limit=12&search=${searchValue}`,
      );
  ...
  const toggleBan = async (id: number) => {
    try {
      const res = await axios.patch(
        `https://bk-production-0584.up.railway.app/participants/participants/${id}/ban`,
      );
